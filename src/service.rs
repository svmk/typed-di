            ...
            &ServiceReference::Owned(..) => {
                ...
                let reference = unsafe {
                    match reference.as_ref() {
                        ...
                        None => {
                            panic!("Unable to deref service `{}`", self.0.instance.get_id());
                        },
                    }
                };
                return reference;
...
impl <T>ServiceAccess<T> for Service<T> {
    fn from_instance(instance: ServiceInstance) -> Result<Service<T>, Error> where T: Any, T: 'static {
        ...
        return Err(Error::service_cast_error(instance.get_id().to_string(), type_name::<T>().to_string(), instance.get_service_type().to_string()))
    }
