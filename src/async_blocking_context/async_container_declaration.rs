...
pub trait AsyncContainerDeclaration {
    fn register_async_blocking<S, Fut, F>(&mut self, service_id: ServiceId<S>, factory: F) -> Result<(), Error>
        where
            F: Fn(&SyncResolver) -> Result<Fut, Error> + Send + Sync + 'static,
            S: Send + Sync + 'static,
            Fut: Future<Output=Result<S, Error>> + Send + 'static;

    fn register_future_blocking<S, F>(&mut self, service_id: ServiceId<S>, future: F) -> Result<(), Error>
        where
